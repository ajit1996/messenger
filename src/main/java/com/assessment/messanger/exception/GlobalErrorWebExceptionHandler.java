...
public class GlobalErrorWebExceptionHandler extends AbstractErrorWebExceptionHandler {
    ...
    private Mono<ServerResponse> renderErrorResponse(ServerRequest request) {
        ...
        return ServerResponse.status(httpStatus).contentType(MediaType.APPLICATION_JSON).body(BodyInserters.fromValue(errorAttributes));
    }

    private static HttpStatus getHttpStatus(Map<String, Object> errorAttributes) {
        HttpStatus httpStatus = errorAttributes.containsKey(MessageConstants.STATUS) ? HttpStatus.valueOf((Integer) errorAttributes.get(MessageConstants.STATUS)) : HttpStatus.BAD_REQUEST;
        return httpStatus;
    }
}